...
export default function Reviews() {
  ...
  useEffect(() => {
    const endpoint = import.meta.env.VITE_REVIEWS_ENDPOINT || '/api/reviews';
    fetch(endpoint)
      .then(r => r.ok ? r.json() : Promise.reject(r.statusText))
      .then((json: ReviewsResponse) => {
        if (json.reviews?.length) setData(json); else setData(FALLBACK);
      })
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-20">
      ...
      <p className="mt-4 text-xs text-slate-500">Live data fetched from Google (falls back to sample). Update place ID or enhance styling as needed.</p>
    </section>
  );
}
